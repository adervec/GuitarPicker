# Credits & content provenance

GuitarPicker bundles **no third-party code, libraries, fonts, images, or audio**. It has
**zero bundled dependencies** and renders all graphics as code-generated SVG. Emoji are drawn
by your operating system's own fonts. The app runs **fully offline by default**; its only
network use is the **optional, off-by-default** Google Drive sync, which loads Google Identity
Services from Google *only when a user chooses to sign in* (see
[docs/CLOUD-SYNC-SETUP.md](./docs/CLOUD-SYNC-SETUP.md)). Sync data lives in the user's own Google
Drive — this project operates **no server** and runs no analytics.

- **Source code** — original work by Adam Eryavec, licensed under the [MIT License](./LICENSE).
- **Glossary** — original definitions written for this project.
- **Songs** — every built-in song is a **traditional or public-domain** musical work, or
  an original practice progression. Only the underlying melodies/chords are used, authored
  here as note data; **no audio recordings or third-party arrangements are bundled.**

> The MIT License covers GuitarPicker's **source code only.** The musical works below are
> in the public domain — they are credited here for transparency and are not claimed under
> the project's copyright.

## Built-in song library

| Song | Credited to | Status |
|------|-------------|--------|
| Twinkle, Twinkle, Little Star | Trad. (melody 1761; lyrics J. Taylor, 1806) | Public domain |
| Ode to Joy | Ludwig van Beethoven (1824) | Public domain |
| When the Saints Go Marching In | Traditional (American gospel) | Public domain |
| Auld Lang Syne | Trad. melody; lyrics Robert Burns (1788) | Public domain |
| Frère Jacques | Traditional (French round, 18th c.) | Public domain |
| Amazing Grace | Lyrics J. Newton (1779); melody "New Britain" (1835) | Public domain |
| Scarborough Fair | Traditional (English ballad) | Public domain |
| Greensleeves | Traditional (16th c.) | Public domain |
| House of the Rising Sun | Traditional (American folk) | Public domain \* |
| Canon in D (excerpt) | Johann Pachelbel (c. 1680) | Public domain |
| Mary Had a Little Lamb | Trad. melody; lyrics S. J. Hale (1830) | Public domain |
| Row, Row, Row Your Boat | Traditional | Public domain |
| London Bridge Is Falling Down | Traditional | Public domain |
| Old MacDonald Had a Farm | Traditional | Public domain |
| Jingle Bells | James Lord Pierpont (1857) | Public domain |
| Oh! Susanna | Stephen Foster (1848) | Public domain |
| Silent Night | F. Gruber / J. Mohr (1818); English trans. (1859) | Public domain |
| Three-Chord Trainer (G–C–D) | Original (GuitarPicker) | Original work |
| Pop Progression I–V–vi–IV | Original (chord progression) | Not copyrightable |
| 12-Bar Blues in A | Traditional blues form | Not copyrightable |
| Ode to Joy (Ukulele) | Ludwig van Beethoven (1824) | Public domain |
| When the Saints (Bass Line) | Traditional | Public domain |
| 12-Bar Blues in E | Traditional blues form | Not copyrightable |
| Doo-Wop Progression I–vi–IV–V (C) | Original chord progression | Not copyrightable |
| Andalusian Cadence (Am–G–F–E) | Traditional Spanish harmonic form | Not copyrightable |
| Pachelbel Strum Loop (D) | Johann Pachelbel (c. 1680), chord loop | Public domain |
| Minor 12-Bar Blues in Am | Traditional blues form | Not copyrightable |
| Jazz ii–V–I Loop (C) | Original chord progression | Not copyrightable |
| Folk Trainer I–IV–V (D) | Original chord progression | Not copyrightable |
| Travis Picking Pattern (C–Am–F–G) | Original (GuitarPicker) | Original work |
| Hot Cross Buns | Traditional (English) | Public domain † |
| Joy to the World | Music attr. G. F. Handel; lyrics I. Watts (1719) | Public domain † |
| We Wish You a Merry Christmas | Traditional (English carol) | Public domain † |
| She'll Be Coming 'Round the Mountain | Traditional (American folk) | Public domain † |
| Oh My Darling, Clementine | Traditional (attr. P. Montrose, 1884) | Public domain † |
| Camptown Races | Stephen Foster (1850) | Public domain † |
| Down in the Valley | Traditional (American folk) | Public domain † |
| Michael, Row the Boat Ashore | Traditional (African-American spiritual) | Public domain † |
| Deck the Halls | Traditional (Welsh air; English lyrics 1862) | Public domain † |
| Kumbaya | Traditional (spiritual) | Public domain † |
| The Muffin Man | Traditional (English) | Public domain † |
| Skip to My Lou | Traditional (American) | Public domain † |
| Yankee Doodle | Traditional (18th century) | Public domain † |
| The First Noel | Traditional (English/Cornish carol) | Public domain † |
| O Christmas Tree | Traditional (German, "O Tannenbaum") | Public domain † |
| Buffalo Gals | "Cool White" / J. Hodges (1844) | Public domain † |
| Home on the Range | B. Higley / D. Kelley (1873) | Public domain † |

\* Only the traditional melody is used. Later recorded arrangements (e.g. The Animals')
are separately copyrighted and are **not** included.

† The musical work is public domain. These melodies are **hand-transcribed** as note data
(tagged `verify-melody` in the source) and should be **spot-checked by ear** before release —
the risk is transcription accuracy, not copyright. Only the underlying public-domain melody
and traditional lyrics are used; no third-party arrangement or recording is included.

## User-provided content

The Song Editor and "Listen & Make" let you import or record your own backing/vocal audio.
That content stays **on your device** (browser `localStorage`) and is never uploaded. You
are responsible for ensuring you have the rights to any audio you import.

## Not affiliated

GuitarPicker is an independent hobby project. It is **not affiliated with, sponsored by, or
endorsed by** Yousician or any other company or product mentioned for comparison.
